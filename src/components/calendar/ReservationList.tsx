...
export default function ReservationList({ reservations, onEdit, onDelete, isDeleting }: ReservationListProps) {
  ...
  return (
    <div className="space-y-4 mt-6 relative">
      <h4 className="text-lg font-medium">{reservations.length === 0 ? 'Aucune réservation pour cette date' : `${reservations.length} réservation(s)`}</h4>
      ...
      {reservations.length > 0 && (
        reservations
          ...
          .map(reservation => (
            <div 
              key={reservation.id}
              className="bg-[#1A1A1A] rounded-lg p-4 border border-[#C4B5A2]/30 flex flex-col sm:flex-row justify-between"
            >
              <div>
